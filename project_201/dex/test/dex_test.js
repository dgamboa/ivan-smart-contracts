...
contract("Dex", (accounts) => {
  // The user must have ETH deposited such that deposited_eth >= buy_order_value
  ...
  // The user must have enough tokens deposited such that token_balance >= sell_order_amount
  ...
  it("should sort the buy order book from highest to lowest", async () => {
    let dex = await Dex.deployed();
    let link = await Link.deployed();
    await link.approve(dex.address, 500);
